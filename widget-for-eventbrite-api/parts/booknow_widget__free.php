...
if ( $data->utilities->get_element( 'booknow', $data->args ) ) {
	...
	switch ( $data->template ) {
		case 'divi':
			$button_markup = '<a %1$s class="submit et_pb_button" %3$s  aria-label="%2$s %5$ %4$s">%2$s</a>';
			break;
		default:
			$button_markup = '<a %1$s class="button btn btn-primary my-3" %3$s  aria-label="%2$s %5$ %4$s">%2$s</a>';
	}
